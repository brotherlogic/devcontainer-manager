...
*   **Pre-Deletion Token Extraction Hook & Closure Reporting:** Prior to container deletion (`devpod delete`), automatically extracts token usage inside the container (`extractTokenUsage`) with a bounded 30-second timeout context to prevent teardown hangs. Supports zero-token and positive counts from both JSON structures (`total_tokens >= 0`) and plain integer strings (`val >= 0`) as well as comprehensive command diagnostics (including exact executed CLI command, container ID, exit code, and stdout/stderr or explicit no-output indicators) on error, updates in-memory configuration state, and posts a markdown token usage report comment (`### 📊 Devcontainer Closure Token Usage Report`) to associated GitHub issues with non-blocking error handling.
*   **Docker Container Liveness & Auto-Recovery:** Verifies active container status directly against the Docker daemon (`docker ps --format '{{.Names}}|{{.Labels}}'`) before considering a DevPod workspace as running. If an underlying Docker container stops, crashes, or is removed (leaving DevPod reporting `NotFound`), DCM treats it as not running and automatically triggers re-provisioning/recreation.
